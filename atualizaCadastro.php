...
//echo $cpf . "<br>";
	...
	if (validaCPF($cpf)){
		...
		$sql_consulta = "UPDATE participante_novo 
						 SET nome = '$nome', sexo = '$sexo', profissao = '$profissao', matricula_emerj = $matEmerj, matricula_tj = $matTj, pais = '$paises', estado = '$uf', email = '$email', celular = '$tel', senha = '$senha', autorizacao = '$autorizacao', data_alteracao = NOW()  
		                 WHERE '$cpf' = cpf"; 
		$sql_result_consulta = mysqli_query($conexao, $sql_consulta) or die(mysqli_error($conexao));
		//$sql_num_rows_consulta = mysqli_affected_rows($sql_result_consulta);
