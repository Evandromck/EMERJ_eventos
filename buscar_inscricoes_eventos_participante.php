  ...
  //'8A5D66280D2DEE3BCC38CE9C404C603C'; //$_POST['txtSenha'];
	...
	$sql = "SELECT DISTINCT 
			...
			FROM 
				 ...
				 frequencia2 f
			WHERE 
			  (i.codParticipante = p.codigo) AND
				(i.codEvento = e.codigo) AND
				(e.codigo = o.codEvento) AND
				(i.codEvento = o.codEvento) AND
				(i.codEvento = f.codEvento) AND
				(i.codParticipante = f.codParticipante) AND
				(f.codParticipante = p.codigo) AND
				(f.percentual >= 0.75) AND
				(o.cargaHoraria <> '00:00:00') AND
				(p.email = '$email') /*AND
...
<form method="post" name="indexcanc" action="imprimecomprovacaodehoraspdf.php">
